...
系统 SHALL 按管道顺序（download → extract_audio → transcribe → analyze → dingtalk_push）依次启动 5 个 worker 子进程。每个 worker 以 `python -m hot_pulse.xxx_worker` 方式启动为独立子进程。主进程 SHALL 记录每个子进程的 PID 和启动状态。
